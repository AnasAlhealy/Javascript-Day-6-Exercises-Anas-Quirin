...
function updateItemCount() {
    let num = 0;
    for (item of cart) {
        num += Number(item.qtty);
    }

    let result = num == 1 ? `${num} item` : `${num} items`;
    document.getElementById('num-items').innerHTML = result;
}
...
function Total() {
    ...
    if (total > 10000) document.getElementById("price").innerHTML = (total * 0.9).toFixed(2) + " € (10% discount)";
